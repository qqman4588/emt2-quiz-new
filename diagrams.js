...
/* ---------- 組裝 ---------- */
function d(t,c,s){ return {t:t,c:c,s:s}; }
...
return {
  ...
  "ch_02":[
    ...
    d("格拉斯哥昏迷指數 GCS","運動反應分數最高也最容易混淆，去皮質與去大腦要分清楚。",gcs())
  ],
  "ch_03":[ d("事故現場車輛停放與警示距離",
      "防撞車在後、救護車在前，是現場安全工作區的基本配置。",sceneParking()) ],
  "ch_t04":[ d("到院前檢傷程序：三級初判 → 五級確認",
      "現場以三級快速分流，送醫前以五級確認；一級病人直接判定不檢索。",triageFlow()) ],
  "ch_04":[
  ...
  "ch_05":[ d("外傷評估流程 XABCDE",
      "大量外出血優先於一切，這是外傷與內科評估最大的差別。",xabcde()),
      d("非外傷評估流程（圖 6-5）",
      "AVPU 意識 → 對痛無反應查頸脈呼吸 → ABC 處置 → 輔助檢查病史 → 依危急度定監測頻率。",nontraumaAssess()),
      d("生命徵象與危急個案門檻速查",
  ...
  "ch_06":[ d("12 導程心電圖胸前導極位置",
      ...
      "現場判斷 DOA → 否則立即 CPR＋AED → SGA 接 EtCO₂ 監測品質與 ROSC → 循環後送。",ohcaFlow()),
      d("呼吸困難處置流程（圖 7-6）",
      "給氧＋半坐臥後聽診鑑別：喘鳴(大氣道)／哮鳴(氣喘COPD)／過度換氣；惡化則正壓通氣。",dyspneaFlow()),
      d("低血壓／休克流程（圖 7-7）",
      "SI>1 辨識休克 → 抬腿保暖給氧 → 過敏性休克打腎上腺素、其他依適應症輸液快送。",shockFlow()),
      d("胸痛處置流程（圖 7-11）",
      "OPQRST＋12 導程 → 疑 STEMI 送 PCI；NTG 前先查四大禁忌；惡化依 OHCA。",chestPainFlow()) ],
  "ch_07":[ d("燒燙傷面積九則定律",
      "成人與兒童的頭頸與下肢比例不同，這是最常見的陷阱題。",ruleOfNines()),
      d("外傷出血性休克流程（圖 8-2）",
      "大出血優先止血 → 判斷休克分級 → 保暖＋許可性低血壓輸液 → 快送外傷中心。",traumaShockFlow()),
      d("傷口處理流程（圖 8-5）",
      "加壓 → 止血帶 → 交界處填塞 → 骨折固定(CMS) → 斷肢隔水低溫保存。",woundCareFlow()) ],
  "ch_08":[ d("小兒評估三角 PAT",
      "三個維度的組合可直接推出臨床臆斷，全異常即為心肺衰竭。",pat()),
      d("新生兒復甦救護流程（圖 9-5）",
      "黃金一分鐘：初評→A 初步處置→B 正壓通氣→校正→依心率分流；口訣「擺吸漏壓、壓吹 3:1」。",neonatalResus()) ],
  "ch_10":[ d("精神疾病救護通用流程（圖 11-4）",
      "叫叫 CABD 初評 → 有無立即威脅 → DEFUSE 降階或團隊約束 → 四點約束標準 → 站位安全。",psychFlow()) ],
  "ch_11":[ d("送醫途中與標準化交班（圖 12-5）",
      "途中監測頻率＋安全帶；抵院交班 內科用 ISBAR、創傷用 IMIST。",transportHandover()) ],
  "ch_17":[ d("TECC 三階段與 MARCH 流程",
      "熱區只止血 → 暖區 MARCH（大出血/氣道/呼吸/循環/低體溫頭傷）→ 冷區重評估與 PACE 後送。",marchFlow()) ],
  "ch_12":[ d("溺水急症流程（圖 13-1）",
      "救起評估 → 非外傷機轉不例行固定頸椎 → OHCA 先給 5 次人工呼吸再 CPR → DOA 標準 → 當心二次溺水。",drowningFlow()),
      d("冷、熱急症流程（圖 13-2）",
